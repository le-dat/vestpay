...
import { cacheKeypair, recoverPasskeyWallet } from "@/integrations/sui/passkey";
...
export function LoginForm() {
  ...
  const handlePasskeyLogin = async (e?: React.FormEvent) => {
    ...
    if (!email) {
      setError("Please enter your email first to recover your wallet");
      return;
    }
  ...
  return (
    <form onSubmit={handlePasskeyLogin} className="space-y-8">
      <motion.div
      ...
      <motion.div
        ...
        className="flex items-center gap-4 py-4 border-l-2 border-black/5 pl-6"
      >
        <button
          type="button"
          onClick={() => setRememberMe(!rememberMe)}
          className={`w-6 h-6 border-[3px] shrink-0 flex items-center justify-center transition-all ${
            rememberMe ? "bg-primary border-primary" : "border-black/20 hover:border-black/40"
          }`}
        >
          {rememberMe && (
            <motion.div
