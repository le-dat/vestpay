...
function TokenIcon({ token, size = "md" }: { token: CetusToken; size?: "sm" | "md" | "lg" }) {
  ...
  if (!token.logoURL || imageError) {
    return (
      <div
        className={`${sizeClasses[size]} rounded-full bg-gradient-to-br from-[#00d084] to-[#00c07a] flex items-center justify-center text-white font-black overflow-hidden`}
      >
  ...
  return (
    <div
      className={`${sizeClasses[size]} rounded-full overflow-hidden bg-white flex items-center justify-center`}
    >
      <img
        src={token.logoURL}
        alt={token.symbol}
...
export default function TokenInput({
  ...
  return (
    <div
      className={`p-8 bg-white dark:bg-[#111827] rounded-[32px] border transition-all duration-300 ${type === "sell" ? "border-[#00d084]/30 shadow-[0_0_20px_rgba(0,208,132,0.05)]" : "border-gray-100 dark:border-white/10 shadow-sm"}`}
    >
          ...
          <div className="flex-shrink-0">
            {onTokenChange ? (
              ...
              <div className="flex items-center gap-3 bg-white dark:bg-white/5 p-2 pr-5 pl-2.5 rounded-full border border-gray-100 dark:border-white/10 shadow-lg shadow-gray-200/50 dark:shadow-none">
                ...
                <span className="font-black text-[20px] text-[#111827] dark:text-white tracking-tight">
                  {token.symbol}
                </span>
                <svg
        ...
        <div className="flex justify-between items-center mt-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[#94a3b8] text-[13px] font-bold">
              Balance:{" "}
              <span className="text-[#111827] dark:text-white font-black">{balance || "-"}</span>
            </span>
            {type === "sell" && (
              <div className="flex items-center gap-1.5 ml-2">
                ...
                <button className="text-[12px] font-black text-[#64748b] hover:text-[#00d084] transition-all">
                  %
                </button>
              </div>
            )}
